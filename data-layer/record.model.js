...
const compareArrays = (a1, a2) => a1.length==a2.length && a1.every((v,i)=> v === a2[i])

const checkAuth = (record, user) => record && compareArrays(record.user.id, mongoose.Types.ObjectId(user).id)
...
RecordSchema.statics = {
   ...
   */
  search({ user, keyword = "", dateGreaterThanEqual = "", dateLessThanEqual = "", skip = 0, limit = 10 } = {}) {
    return this.find({
      $and:[
        { description: { $regex: `^${keyword}` }},
        { updatedAt: { $gte: new Date(dateGreaterThanEqual) }},
        { updatedAt: { $lte: new Date(dateLessThanEqual) }},
        { user },
      ],
    })
    .sort({ updatedAt: -1 })
